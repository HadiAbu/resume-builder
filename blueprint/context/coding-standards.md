...
> Your conventions. Edit these once to match your stack. Tuned for a plain
> Next.js 16 (App Router) + React 19 + TypeScript + Tailwind v4 project with no
> database, auth, or validation library chosen yet - add those sections back in
> once the stack in `project-plan.md` picks them.
>
...
- No component library installed yet; add shadcn/ui (or another) here once chosen
...
- Validate all inputs at the boundary (Server Actions, API routes) once a
  validation library is chosen - none is installed yet
- If auth is added, scope every user-owned query by the authenticated user id;
